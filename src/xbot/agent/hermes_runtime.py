...
def _ensure_hermes_import_path() -> Path:
    ...
    from xbot.agent.tools.hermes_weiban_knowledge import register_xbot_weiban_knowledge_tools

    register_xbot_wechat_tools()
    register_xbot_qq_tools()
    register_xbot_telegram_tools()
    register_xbot_weiban_tools()
    register_xbot_weiban_knowledge_tools()
...
def _toolsets_for_source(source: str) -> list[str]:
    ...
    if profile == "guest":
        return [channel_toolset, "weiban-readonly", "weiban-knowledge", "artifacts"]
    if profile == "member":
...
def _tool_policy_denial(function_name: str, function_args: dict[str, Any], policy: dict[str, Any]) -> str | None:
    ...
    if profile == "guest":
        ...
        allowed.add("weiban_search_knowledge")
