"""xbot-native WeChat tools registered into the embedded Hermes runtime."""
...
def send_link(args: dict[str, Any], **_: Any) -> str:
    ...
    return _send(
        ...
        args,
        content=url,
        metadata={"content_xml": _link_xml(args), "content_type": 5},
    )
...
def _music_xml(args: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"<title>{title}</title><des>{singer}</des><action>view</action><type>3</type>"
        f"<showtype>0</showtype><content/><url>{url}</url><dataurl>{music_url}</dataurl>"
        f"<lowurl>{url}</lowurl><lowdataurl>{music_url}</lowdataurl><thumburl>{cover}</thumburl>"
        f"<songlyric>{lyric}</songlyric><songalbumurl>{cover}</songalbumurl>"
        f"<appattach><totallen>0</totallen><attachid/><emoticonmd5/><fileext/>"
        f"<cdnthumburl>{cover}</cdnthumburl><cdnthumbheight>100</cdnthumbheight>"
        "<cdnthumbwidth>100</cdnthumbwidth></appattach><weappinfo><pagepath/><username/>"
        "<appid/><appservicetype>0</appservicetype></weappinfo><websearch/></appmsg>"
        "<fromusername></fromusername><scene>0</scene>"
    )
...
def register_xbot_wechat_tools() -> None:
    """Register xbot tools after the embedded Hermes path is available."""
    from tools.registry import registry

    definitions = [
        (
            "wechat_send_text",
            _schema("wechat_send_text", "text"),
            send_text,
            "Send WeChat text through the current xbot adapter.",
        ),
        (
            "wechat_send_image",
            _schema("wechat_send_image", "image"),
            send_image,
            "Send a WeChat image through the current xbot adapter.",
        ),
        (
            "wechat_send_file",
            _schema("wechat_send_file", "file"),
            send_file,
            "Send a WeChat file through the current xbot adapter.",
        ),
        (
            "wechat_send_voice",
            {
                "name": "wechat_send_voice",
                "description": "Send a WeChat voice message.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "to_wxid": {"type": "string"},
            ...
            send_voice,
            "Send a WeChat voice message through xbot.",
        ),
        (
            "wechat_send_video",
            _schema("wechat_send_video", "video"),
            send_video,
            "Send a WeChat video through xbot.",
        ),
        (
            "wechat_send_link",
            {
                "name": "wechat_send_link",
                "description": "Send a WeChat link card.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "to_wxid": {"type": "string"},
            ...
            send_link,
            "Send a WeChat link card through xbot.",
        ),
        (
            "wechat_send_music_card",
            {
                "name": "wechat_send_music_card",
                "description": "Send a WeChat music card.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "to_wxid": {"type": "string"},
                    ...
                    "required": ["title", "music_url"],
                },
            },
            send_music_card,
            "Send a WeChat music card through xbot.",
        ),
    ]

    for name, schema, handler, description in definitions:
        existing = registry.get_entry(name)
        if existing is not None and existing.handler is handler and existing.toolset == "wechat":
            continue
        registry.register(
            name=name,
            toolset="wechat",
