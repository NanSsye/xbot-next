...
from database import group_members_db as group_members_db_module
...
group_members_db_module.DB_PATH = GROUP_MEMBERS_DB_PATH

class OpenClawBridge(PluginBase):
    ...
    async def handle_text(self, bot, message):
        ...
        if should_forward:
             ...
             if is_group and self.limits_config.get("enable_group_limit", False):
                 ...
                 if sender_wxid in whitelist:
                     ...
                     try:
                         ...
                         stored = await kvdb.get(db_key)
                         if stored:
                             import ast
                             record = ast.literal_eval(stored)
                     ...
                     if record["count"] >= limit:
                         ...
                         if record["count"] == limit:
                             ...
                             # 保存到数据库
                             try:
                                 remaining = max(1, int(record["reset_time"] - now))
                                 await kvdb.set(db_key, str(record), ex=remaining)
                             except Exception:
                                 pass
                         return
             ...
             if media_data and not quoted_text_content and isinstance(media_data, dict):
