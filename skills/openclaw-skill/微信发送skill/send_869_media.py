...
# Add VENDOR_DIR's parent so 'vendor' can be imported as a package
VENDOR_PARENT = VENDOR_DIR.parent
if VENDOR_PARENT.exists() and str(VENDOR_PARENT) not in sys.path:
...
DEFAULT_CONFIG_PATH = Path("/home/nans/.openclaw/credentials/wechat-869.json")
...
def build_music_appmsg_xml(
    *,
    ...
    cover_url: str = "",
    lyric: str = "",
    appid: str = "wx79f2c4418704b4f8",
    sdkver: str = "0",
) -> str:
    """构建微信音乐卡片 XML（type=3）。"""
    return (
        f'<appmsg appid="{xml_escape(appid)}" sdkver="{xml_escape(sdkver)}">'
        f'<title>{xml_escape(title)}</title>'
        f'<des>{xml_escape(singer)}</des>'
        f'<action>view</action>'
        f'<type>3</type>'
        f'<showtype>0</showtype>'
        f'<content/>'
        f'<url>{xml_escape(url)}</url>'
        f'<dataurl>{xml_escape(music_url)}</dataurl>'
        f'<lowurl>{xml_escape(url)}</lowurl>'
        f'<lowdataurl>{xml_escape(music_url)}</lowdataurl>'
        f'<thumburl>{xml_escape(cover_url)}</thumburl>'
        f'<songlyric>{xml_escape(lyric)}</songlyric>'
        f'<songalbumurl>{xml_escape(cover_url)}</songalbumurl>'
        f'<appattach>'
        f'<totallen>0</totallen>'
        f'<attachid/>'
        f'<emoticonmd5/>'
        f'<fileext/>'
        f'<cdnthumburl>{xml_escape(cover_url)}</cdnthumburl>'
        f'<cdnthumbheight>100</cdnthumbheight>'
        f'<cdnthumbwidth>100</cdnthumbwidth>'
        f'</appattach>'
        f'<weappinfo>'
        f'<pagepath/>'
        f'<username/>'
        f'<appid/>'
        f'<appservicetype>0</appservicetype>'
        f'</weappinfo>'
        f'<websearch/>'
        f'</appmsg>'
...
def send_music_card(
    ...
    *,
    ...
    """发送微信音乐卡片（通过 AppMessage XML，type=3）。"""
...
def send_text(cfg: ClientConfig, *, to_wxid: str, text: str, at_wxids: list[str]) -> Any:
    payload = {
        "MsgItem": [
            {
                "ToUserName": to_wxid,
...
def _audio_chunk_to_silk_payload(audio_chunk) -> tuple[bytes, int, int]:
    try:
        if pysilk is None:
            raise RuntimeError("缺少 pysilk 依赖")
        ...
        raw_bytes = audio_chunk.raw_data
        # fallback 时 voice_seconds 用音频实际时长，不用字节数推算
        derived_seconds = max(1, int((len(audio_chunk) + 999) // 1000))
        return raw_bytes, 4, min(MAX_VOICE_SECONDS, derived_seconds)
...
def _prepare_voice_payloads(voice_path: Path, fmt: str, seconds: int) -> list[dict[str, Any]]:
    ...
    audio = _load_audio_segment(voice_path, fmt_normalized)
    actual_seconds = max(1, int((len(audio) + 999) // 1000))
    # 只有在 pysilk 不可用时才发原始字节（mp3/wav 需要转 SILK 才能在微信播放）
    if pysilk is None:
        return [{
            "voice_bytes": read_bytes(voice_path),
            "voice_format": 4,
            "voice_seconds": actual_seconds,
            "wire_codec": fmt_normalized,
        }]

    # pysilk 可用：所有音频都转 SILK 再发
    payloads = []
    for chunk in _slice_audio_segment(audio):
        voice_bytes, voice_format, voice_seconds = _audio_chunk_to_silk_payload(chunk)
        payloads.append({
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_music_card = sub.add_parser("send-music-card", help="发送微信音乐卡片（非文本）")
    p_music_card.add_argument("--to", required=True, help="接收人 wxid（群聊一般以 @chatroom 结尾）")
    p_music_card.add_argument("--title", required=True, help="歌曲名")
    p_music_card.add_argument("--singer", required=True, help="歌手")
    p_music_card.add_argument("--url", required=True, help="分享链接 URL")
    p_music_card.add_argument("--music-url", required=True, help="音频直链")
    p_music_card.add_argument("--cover-url", default="", help="封面图 URL（可选）")
    ...
    p_text = sub.add_parser("send-text", help="发送文本消息")
    p_text.add_argument("--to", required=True, help="接收人 wxid（群聊一般以 @chatroom 结尾）")
    p_text.add_argument("--text", required=True, help="文本内容")
    p_text.add_argument("--at", action="append", default=[], help="可重复：@某个 wxid")
...
def main(argv: list[str]) -> int:
    ...
    if cmd == "send-text":
        text = str(args.text)
        at = [str(x).strip() for x in (getattr(args, "at", []) or []) if str(x).strip()]
        result = send_text(cfg, to_wxid=to_wxid, text=text, at_wxids=at)
        _print_result(result)
        return 0
