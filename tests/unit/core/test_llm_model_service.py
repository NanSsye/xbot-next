...
async def test_discovers_sorted_unique_openai_models_without_exposing_key(monkeypatch):
    ...
    config = AgentLLMConfig(api_key=secret, base_url="https://models.example.test/v1", model="model-a", enabled_models=["model-a"])

    assert await LLMModelDiscoveryService().discover(config) == ["model-a", "model-b"]
...
async def test_anthropic_root_url_uses_v1_models_and_key_header(monkeypatch):
    ...
    install_transport(monkeypatch, handler)
    config = AgentLLMConfig(provider="anthropic", api_key="anthropic-key", base_url="https://api.anthropic.com", model="claude-model", enabled_models=["claude-model"])
